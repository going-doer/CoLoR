...
def compute_f1_at_k(
    ...
    recall = compute_recall_at_k(gold_ids, pred_ids, top_k, capped)
    precision = compute_precision_at_k(gold_ids, pred_ids, top_k)
...
class RetrievalEvaluation(evaluation.LOFTEvalution):
  ...
  def evaluate(self, instance: EvaluationInstance) -> list[dict[str, Any]]:
    ...
    for turn_number in range(instance.num_turns):
      ...
      # minju
      instance_metrics["precision@1"] = compute_precision_at_k(
        gold_ids, pred_ids, 1
      )
      instance_metrics["precision@2"] = compute_precision_at_k(
        gold_ids, pred_ids, 2
      )
      instance_metrics["precision@3"] = compute_precision_at_k(
        gold_ids, pred_ids, 3
      )
      instance_metrics["precision@5"] = compute_precision_at_k(
        gold_ids, pred_ids, 5
      )

      instance_metrics["f1@1"] = compute_f1_at_k(
        gold_ids, pred_ids, 1, False
      )
      instance_metrics["f1@2"] = compute_f1_at_k(
        gold_ids, pred_ids, 2, False
      )
      instance_metrics["f1@3"] = compute_f1_at_k(
        gold_ids, pred_ids, 3, False
      )
      instance_metrics["f1@5"] = compute_f1_at_k(
        gold_ids, pred_ids, 5, False
      )
